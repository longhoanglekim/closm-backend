...
import longhoang.uet.mobile.closm.services.AuthenticationService;
...
public class AuthenticationController {
    ...
    public ResponseEntity<?> authenticateAdmin(@RequestBody LoginInput input) {
        try {
            User user = authenticationService.authenticateAdmin(input);
            String token = jwtUtil.generateToken(new UserDetailsImpl(user));
            LoginResponse loginResponse = new LoginResponse();
            loginResponse.setToken(token);
            loginResponse.setEmail(user.getEmail());
